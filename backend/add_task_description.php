...
//NEW INSERT
//$stmtInsert = $conn->prepare("INSERT INTO task_descriptions (work_mode_id, description, billing_category_id)VALUES (?, ?, ?)");
...
foreach ($descriptions as $task) {

    $desc = trim($task['description']);
    $billingCategory = $task['billing_category_id'] ?? null;
    $standardAht = $task['standard_aht'] ?? null;
